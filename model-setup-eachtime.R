...
entire_pop <- merge(setDT(entire_pop), waning_data_clean, by.x = c("age_group", "prior_inf", "immunocompromised", "weeks_since_last_dose_inf"), by.y = c("age_group", "prior_inf", "immunocompromised", "weeks"), all.x = TRUE)

#Adjust protection
  #Individuals who are unvaccinated and no prior infection history has no protection
immune_naive_index <- which(entire_pop$num_doses=="unvax" & entire_pop$prior_inf == 0)
entire_pop[immune_naive_index, c("severe_ve_pred", "nonsevere_ve_pred")] <- 0

  #Individuals who are unvaccinated and have prior infection history have prior infection only waning immunity
prior_inf_only_index <- which(entire_pop$num_doses=="unvax" & entire_pop$prior_inf == 1)
entire_pop[prior_inf_only_index, c("severe_ve_pred", "nonsevere_ve_pred")] <- entire_pop[prior_inf_only_index, c("severe_prior_inf_only_ve_pred", "nonsevere_prior_inf_only_ve_pred")]

  #Individuals with infection <3 months from simulation start has perfect immunity
perfect_immunity_index <- which(entire_pop$weeks_since_last_inf<13 | entire_pop$weeks_since_last_reinf<13)
entire_pop[perfect_immunity_index, c("severe_ve_pred", "nonsevere_ve_pred")] <- 1

pe_by_age <- entire_pop %>%
  group_by(age_group) %>%
...
inf_by_age <- merge(entire_pop %>% group_by(age_group, immunocompromised) %>% summarise(total_pop = n()),
...
severe_infection_multipliers <- setDT(entire_pop %>% 
...
waning_data_clean <- merge(merge(merge(waning_data_clean, severe_infection_multipliers, by = c("age_group", "immunocompromised"), all.x = TRUE),
